...
const navItems = [
  { label: "My Profile", href: "/candidate", icon: UserCircleIcon, exact: true },
  { label: "Resume", href: "/candidate/resume", icon: FileTextIcon },
  { label: "Skills", href: "/candidate/skills", icon: CodeIcon },
  { label: "Experience", href: "/candidate/experience", icon: BriefcaseIcon },
  { label: "Education", href: "/candidate/education", icon: GraduationCapIcon },
  { label: "Certifications", href: "/candidate/certifications", icon: AwardIcon },
];
...
export default function CandidateLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (stored.role !== "candidate") {
      router.replace(stored.role === "sourcer" ? "/sourcer" : "/login");
      return;
    }
    ...
    <div className="flex h-screen bg-slate-50">
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <div className="flex md:hidden items-center gap-3 px-4 py-3 bg-slate-900 border-b border-slate-700">
          ...
          <span className="text-white font-semibold">Candidate Portal</span>
        </div>
